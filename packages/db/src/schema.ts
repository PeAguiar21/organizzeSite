import { mysqlTable, serial, varchar, timestamp, decimal, mysqlEnum, date, text, json, int, type AnyMySqlColumn } from "drizzle-orm/mysql-core";
...
export const accounts = mysqlTable("accounts", {
  ...
  type: mysqlEnum("type", ["WALLET", "CHECKING", "SAVINGS", "INVESTMENT"]).default("CHECKING"),
...
export const transactions = mysqlTable("transactions", {
  id: serial("id").primaryKey(),
  userId: int("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  accountId: int("account_id").notNull().references(() => accounts.id, { onDelete: "restrict" }),
...
export const auditLogs = mysqlTable("audit_logs", {
  ...
  action: mysqlEnum("action", ["CREATE", "UPDATE", "DELETE", "LOGIN"]).notNull(),
